...
def test_decode_ruleset_terrain_control_full_packet():
    ...
    payload = bytes([
        0xd3, 0x0c,  # Bitvector: bits 0,1,4,6,7,10,11 set
        0x1e,        # Bit 0: ocean_reclaim_requirement_pct = 30
        0x0a,        # Bit 1: land_channel_requirement_pct = 10
        0x0e,        # Bit 4: lake_max_size = 14
        0x00, 0x00, 0x00, 0x06,  # Bit 6: move_fragments = 6 (big-endian)
        0x00, 0x00, 0x00, 0x02,  # Bit 7: igter_cost = 2 (big-endian)
        ...
        0x3f, 0x67, 0x75, 0x69, 0x5f, 0x74, 0x79, 0x70, 0x65, 0x3a, 0x42, 0x75, 0x69, 0x6c, 0x64,
        0x20, 0x46, 0x6f, 0x72, 0x74, 0x2f, 0x46, 0x6f, 0x72, 0x74, 0x72, 0x65, 0x73, 0x73, 0x2f,
        0x42, 0x75, 0x6f, 0x79, 0x00,  # "?gui_type:Build Fort/Fortress/Buoy\0"
        # Bit 11: gui_type_base1 (string)
        0x3f, 0x67, 0x75, 0x69, 0x5f, 0x74, 0x79, 0x70, 0x65, 0x3a, 0x42, 0x75, 0x69, 0x6c, 0x64,
        0x20, 0x41, 0x69, 0x72, 0x73, 0x74, 0x72, 0x69, 0x70, 0x2f, 0x41, 0x69, 0x72, 0x62, 0x61,
        0x73, 0x65, 0x00,  # "?gui_type:Build Airstrip/Airbase\0"
    ])
...
def test_decode_ruleset_terrain_control_boolean_header_folding():
    ...
    payload = bytes([
        0x00, 0x03,  # Bitvector: bits 8,9 set
...
def test_decode_ruleset_terrain_control_delta_protocol():
    ...
    payload1 = bytes([
        0xff, 0x0f,  # Bitvector: all 12 bits set
        ...
        0, 0, 0, 6,  # move_fragments (big-endian)
        0, 0, 0, 2,  # igter_cost (big-endian)
        # Bits 8,9 are True (in bitvector)
        ord('A'), 0,  # gui_type_base0 = "A"
        ord('B'), 0,  # gui_type_base1 = "B"
    ])
...
def test_decode_ruleset_terrain_control_uint32_fields():
    ...
    payload = bytes([
        0xc0, 0x00,  # Bitvector: bits 6,7 set
        0x12, 0x34, 0x56, 0x78,  # move_fragments = 0x12345678 (big-endian)
        0xab, 0xcd, 0xef, 0x01,  # igter_cost = 0xabcdef01 (big-endian)
    ])
...
def test_decode_ruleset_terrain_control_empty_strings():
    ...
    payload = bytes([
        0x00, 0x0c,  # Bitvector: bits 10,11 set
