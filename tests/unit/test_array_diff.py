...
class TestArrayDiffBasic:
    ...
    def test_decode_multiple_changes_uint8_indices(self):
        ...
        data = bytes([
            10, 1,    # index=10, value=True
            20, 1,    # index=20, value=True
            30, 0,    # index=30, value=False
            50        # sentinel
        ])
    ...
    def test_decode_with_cache(self):
        ...
        data = bytes([
            1, 1,     # index=1, value=True (was False)
            3, 1,     # index=3, value=True (was False)
            5         # sentinel
        ])
    ...
    def test_decode_uint16_indices(self):
        ...
        data = bytes([
            1, 0,     # index=256 (big-endian uint16)
            1,        # value=True (BOOL is 1 byte)
            1, 44     # sentinel=300 (big-endian uint16)
        ])
...
class TestArrayDiffElementTypes:
    ...
    def test_decode_sint32_elements(self):
        ...
        data = bytes([
            ...
            0, 0, 0, 42,    # value=42 (SINT32 big-endian)
    ...
    def test_decode_uint16_elements(self):
        ...
        data = bytes([
            ...
            3, 232,     # value=1000 (UINT16 big-endian)
...
class TestArrayDiffEdgeCases:
    ...
    def test_decode_last_element(self):
        ...
        data = bytes([
            9, 1,     # index=9 (last valid index for size 10), value=True
            10        # sentinel
        ])
    ...
    def test_decode_invalid_index(self):
        ...
        data = bytes([
            11, 1,    # index=11 (invalid!), value=True
            10        # sentinel
        ])
    ...
    def test_decode_with_offset(self):
        ...
        data = bytes([99, 88, 77,  # prefix (ignored)
                      5, 1, 10])    # array-diff: index=5, value=True, sentinel=10
    ...
    def test_decode_preserves_cache_unmodified_indices(self):
        ...
        data = bytes([
            2,              # index=2 (uint8)
            0, 0, 0, 99,    # value=99 (SINT32 big-endian)
            5               # sentinel=5 (uint8)
        ])
    ...
    def test_decode_wrong_cache_size_reinitializes(self):
        ...
        data = bytes([
            2, 1,     # index=2, value=True
            5         # sentinel for size 5
        ])
